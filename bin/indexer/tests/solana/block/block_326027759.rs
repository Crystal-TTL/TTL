...
async fn test_ok() {
    ...
    run_test_with_pool_on_empty_db(|pool| async move {
		...
		assert_invalid_utf8_name(&mut tx).await;
		assert_token_balance(&mut tx).await;
	})
		.await
}

async fn assert_invalid_utf8_name<'a>(tx: &mut Tx<'a>) {
    let token = TokenRepo::new_read_only()
        .get_by_mint(tx, "DnLM31nU9K4kczFoAfhbU4hrK7XaAxSJfkieUqnPpump")
        .await
        .unwrap();
    ...
    assert_eq!(
        token.block_time.unwrap().deref(),
        "2025-03-11 10:13:53.0 +00:00:00"
    );
}

async fn assert_token_balance<'a>(tx: &mut Tx<'a>) {
    // TX: 5YLxRB9Wm3CR7nrU6svVZ5RLrPiXUYJz57dJcqZvM4Uhmgonq3Kg9dhS4iDE5q55M3PTKYfV75RFDyNGiD2SyAdd
    assert_sql!(
        tx,
        r#"
		(
			select balance  from solana.token_balance b
				join solana.token t on t.id = b.token_id
				join solana.address a on a.id = b.address_id
				where mint = 'FGYgFJSxZTGzaLwzUL9YZqK2yUZ8seofCwGq8BPEw4o8' and address = 'CbYf9QNrkVgNRCMTDiVdvzMqSzXh8AAgnrKAoTfEACdh'
		) = '664.58953402'
		"#
    );
}
