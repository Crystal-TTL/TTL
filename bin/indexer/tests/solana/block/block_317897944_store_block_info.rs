...
insert into solana.token (mint, name, symbol, decimals, supply, metadata, description, image, website, creator_id, block_id, block_time) values
		...
		assert_sql!(&mut tx, "(select slot from solana.block) = 317897944");
		assert_sql!(&mut tx, "(select timestamp from solana.block) = '2025-02-02 02:23:39.000000 +00:00'");
		assert_sql!(&mut tx, "(select hash from solana.block) = '2r5UFMTrkeRzE9CPoK4vd8Bw9jPfgRBiHAmErX3jFKUP'");
	})
		.await
}
