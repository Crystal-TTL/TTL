...
use base::testing::user::create_telegram_user;
use base::testing::wallet::create_wallet;
use common::repo::Tx;
...
async fn test_ok() {
    // test pretends that some wallets in this real block belong to nyanbot user
    // to test wallet_balance_token and wallet_balance_sol
    ...
    run_test_with_pool_on_empty_db(|pool| async move {
		...
		let _ = create_telegram_user(&mut tx, 1).await;
		let _ = create_wallet(&mut tx, 1).await;
		let _ = create_telegram_user(&mut tx, 2).await;
		let _ = create_wallet(&mut tx, 2).await;
		tx.commit().await.unwrap();


		pool.acquire().await.unwrap().execute(r#"
		update solana.wallet set public_key = '4e3nd25C5Mn4oMDDyRazeG1UaydWCxnG5c7eL2D7VAAb' where user_id = 1;
		update solana.wallet set public_key = '7jmimzvb73PUayxNzxGmfmX8p53ovSdJQUNZ4QcTabCg' where user_id = 2;

		insert into solana.address (address) values  ('Gb4ZDCisirfKPRxKDXqXSVwSJHv9sV415Ew3zN55UaFw');
		...
		let mut tx = pool.begin().await.unwrap();
		assert_token(&mut tx).await;
		assert_sol(&mut tx).await;
	})
		.await
}

async fn assert_token<'a>(tx: &mut Tx<'a>) {
    assert_sql!(
        tx,
        "(select slot from solana.wallet_balance_token where wallet_id = 2) = '326027759'"
    );

    assert_sql!(
        tx,
        "(select timestamp from solana.wallet_balance_token where wallet_id = 2) = '2025-03-11 10:13:53'"
    );

    assert_sql!(
        tx,
        "(select balance from solana.wallet_balance_token where wallet_id = 2) = '1.529450274000'"
    );
    assert_sql!(
        tx,
        "(select delta from solana.wallet_balance_token where wallet_id = 2) = '0.000016415000'"
    );
}

async fn assert_sol<'a>(tx: &mut Tx<'a>) {
    assert_sql!(
        tx,
        "(select balance from solana.wallet_balance_sol where wallet_id = 1) = '0.014865303000'"
    );

    assert_sql!(
        tx,
        "(select delta from solana.wallet_balance_sol where wallet_id = 1) = '-0.000300001000'"
    );

    assert_sql!(
        tx,
        "(select slot from solana.wallet_balance_sol where wallet_id = 1) = '326027759'"
    );

    assert_sql!(
        tx,
        "(select timestamp from solana.wallet_balance_sol where wallet_id = 1) = '2025-03-11 10:13:53'"
    );
}
