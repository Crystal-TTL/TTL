...
use crate::config::PumpupProgressConfig;
...
#[async_trait]
impl Worker<Partition> for ProgressWorker {
    async fn process(&self, partition: Partition) {
        let repo = CandleRepo::new();
...
pub fn pumpup_progress(cfg: PumpupProgressConfig, signal: Signal) -> JoinHandle<()> {
    spawn(async move {
