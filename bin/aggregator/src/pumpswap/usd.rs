...
use crate::config::PumpswapUsdConfig;
...
use log::{info, warn};
use solana::pumpswap::repo::CandleRepo;
use sqlx::PgPool;
use std::sync::Arc;
...
#[async_trait]
impl Worker<Partition> for UsdWorker {
...
pub fn pumpswap_usd(cfg: PumpswapUsdConfig, signal: Signal) -> JoinHandle<()> {
    spawn(async move {
        ...
        spawn(partitioned(signal, receivers, Arc::new(UsdWorker { pool })));
